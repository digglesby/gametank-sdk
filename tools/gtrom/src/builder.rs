...
impl ElfSection {
    fn from_static(section_name: String, elf: &ElfBytes<'_, AnyEndian>, bank: u8) -> Option<Self> {
        let header = elf.section_header_by_name(&section_name).expect("something failed")?;

        let load_addr = header.sh_addr as usize;
        let size = header.sh_size as usize;
        let offset_in_bank = load_addr & 0x3FFF;

        let (d, ch) = elf.section_data(&header).expect("bruh");

        Some(
            Self {
                display_name: demangle(&section_name).to_string(),
                internal_name: section_name,
                bytes: Vec::from(d),
                size,
                mem_loc: load_addr,
                bank,
    ...
    fn from_loaded(section_name: String, elf: &ElfBytes<'_, AnyEndian>, load_symbol: String) -> Option<Self> {
...
impl RomBuilder {
    pub fn init(path: String) -> Self {
        ...
        // 128 banks
        let mut static_sections: [Vec<String>; 128] = std::array::from_fn(|i|
            match i {
                0..=126 => vec![
                    format!(".text.bank{}", i), 
        ...
        let map_sections: Vec<ElfSection> = static_sections
            ...
            .flat_map(|(bank, names)| {
                names.iter().filter_map(move |name| {
                        ElfSection::from_static(name.clone(), elf, bank as u8)
                    }
                )
            }
        )
        ...
        for s in map_sections {
            ...
            println!("{:<24}bank {} @{:04X}..{:04X} ${:04X}", s.display_name, s.bank, s.bank_loc, s.bank_loc + s.size, s.mem_loc);
        }
